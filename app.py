...
# 1. Konfigurasi Halaman & Background Berwarna
st.set_page_config(page_title="Tubes Mekflu - Kelompok Pompa", page_icon="💧", layout="wide")

# CSS untuk Background Berwarna dan Styling
st.markdown("""
    <style>
    .stApp {
...
st.markdown("<h1 class='main-title'>🚜 Dashboard Simulasi Pompa Air</h1>", unsafe_allow_stdio=True)
st.markdown("<p style='text-align: center;'>Tugas Besar Mekanika Fluida - Teknik Fisika</p>", unsafe_allow_stdio=True)
...
with st.sidebar:
    ...
    st.markdown("---")
    st.info("Gunakan slider untuk melihat perubahan daya secara real-time.")

# 3. Perhitungan Logika
rho = 1000  # kg/m3
g = 9.81    # m/s2
...
with col_anim:
    ...
    for i in range(50):
        ...
        # Partikel Air Bergerak (Kecepatan sesuai Debit)
        t = (i * debit * 8) % 10
        ax_anim.plot([(t) % 4], [5], 'bo', ms=10, alpha=0.7) # Partikel Masuk
        ax_anim.plot([5], [6 + (t % 4)], 'bo', ms=10, alpha=0.7) # Partikel Keluar
...
with col_data:
    ...
    st.markdown("<div class='metric-container'>", unsafe_allow_stdio=True)
    st.metric(label="Estimasi Daya Listrik", value=f"{daya_kw:.2f} kW")
    
    # Status Daya
    if daya_kw > 20:
        st.error("⚠️ STATUS: DAYA TINGGI")
        st.write("Sistem memerlukan konsumsi energi yang besar.")
    else:
        st.success("✅ STATUS: DAYA NORMAL")
        st.write("Sistem bekerja pada beban operasional standar.")
...
# 5. Grafik Karakteristik (Paling Bawah)
st.markdown("---")
...
ax_graph.plot(q_range, p_range, color='#00796b', label='Kurva Kebutuhan Daya', lw=2)
ax_graph.fill_between(q_range, p_range, color='#b2dfdb', alpha=0.3)
ax_graph.scatter(debit, daya_kw, color='red', s=100, label='Titik Operasi Anda', zorder=5)
...
st.caption("Dikembangkan untuk Tugas Besar Mekanika Fluida 2026")
